...
export default class Scanner {
  ...
  public scan(): Token {
    try {
      ...
      let trio: TokenType;
      if (
        (trio = triple.get(String.fromCodePoint(char, this.current, this.peek)))
      ) {
        this.advance();
        this.advance();
  ...
  private identifier(): Token {
    ...
    return this.makeToken(keywords.get(this.literal) || TokenType.identifier);
  }
  ...
  private number(previous: number): Token {
    ...
    const readDigits = (allowUnderscore: boolean) => {
      ...
      if (!allowUnderscore || isAlpha(this.current) || isDigit(this.current))
        this.throwErrorToken("malformed number");
    };
    ...
    if (base === 10)
      if (this.current === codeDot) {
        ...
        if (!isDigit(this.current)) this.throwErrorToken("malformed number");
        readDigits(false);
      } else if (this.current === codeOf("f") || this.current === codeOf("F")) {
        type = TokenType.float;
        this.advance();
      }
